...
export function MyNftsTab() {
  ...
  const listedByMe = useMemo(() => {
    ...
    return allTokenIds
      .map((id, index) => {
        ...
        if (!isListed || seller.toLowerCase() !== address.toLowerCase()) {
          return null;
        }
  ...
  const myTokenIds = useMemo(() => {
    const unique = new Map<
      string,
      { tokenId: bigint; isListed: boolean; price?: bigint }
    >();
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {myTokenIds.map(({ tokenId }, index) => {
        ...
        const isListed =
          listingResult?.status === "success" ? listingResult.result[2] : false;
        const price =
